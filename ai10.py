...
def check_winner(board): 
    for row in board: 
        if row[0] == row[1] == row[2] != EMPTY: return row[0] 
    for col in range(3): 
        if board[0][col] == board[1][col] == board[2][col] != EMPTY: return board[0][col] 
...
def play_game(): 
    ...
    while True: 
        player_move(board) 
        print_board(board) 
        if check_winner(board) == PLAYER: print("Player wins!"); break 
        if is_full(board): print("It's a tie!"); break 
         
        ai_move = find_best_move(board) 
        board[ai_move[0]][ai_move[1]] = AI 
        print_board(board) 
        if check_winner(board) == AI: print("AI wins!"); break 
        if is_full(board): print("It's a tie!"); break 
